...
fn parse_movie_id(path: &Path) -> usize {
...
pub fn load_netflix_dataset(n_movies: usize) -> NetflixData<'static> {
    ...
    NetflixData {
        matrix: m,
        iter: m.iter().peekable(),
    }
}

pub struct NetflixData<'a> {
    matrix: NetflixMatrix,
    iter: Peekable<hash_map_spmat::Iter<'a, f32>>,
}

impl<'a> DataLoader<((usize, usize), f32)> for NetflixData<'a> {
    fn len(&self) -> usize {
        self.matrix.nnz()
    }

    fn start_new_epoch(&mut self) {
        self.iter = self.matrix.iter().peekable()
    }

    fn take(&mut self, n: usize) -> (Vec<((usize, usize), f32)>, bool) {
        let data = self.iter.take(n).collect();
